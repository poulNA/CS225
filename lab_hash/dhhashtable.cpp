...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    size_t index1 = hashes::hash(key, size);
    size_t index2 = hashes::secondary_hash(key, size);

    while (should_probe[index1]) {
        index1 = (index1 + index2) % size;
    }

    table[index1] = new std::pair<K, V>(key, value);
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while (should_probe[index1]) {
        if (table[index1] != NULL && table[index1]->first == key) {return index1;}
        index1 = (index1 + index2) % size;
        if (index1 == start) {return -1;}
    }
    return -1;
}

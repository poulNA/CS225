...
class KDTree
{
  ...
  private:
     ...
     */
    void copy(KDTreeNode * curr);
    void destroy(KDTreeNode * root);
    void buildTree(vector<Point<Dim>> & points, int dim, int left, int right, KDTreeNode *& curRoot);
    Point<Dim> quickselect(vector<Point<Dim>> & vect, int left, int right, int k, int dim);
    int partition(vector<Point<Dim>> & vect, int left, int right, int pivot, int dim);
    Point<Dim> findNNHelper(const Point<Dim> query, int dim, KDTreeNode * curr, Point<Dim> nearest) const;
};
